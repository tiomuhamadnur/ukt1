...
class CutiDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('#', function ($item) {
                ...
                $printButton = "<a href='javascript:;' class='btn btn-outline-info'
                                    title='Print PDF' title='Download PDF' data-toggle='modal'
    ...
    public function query(Cuti $model): QueryBuilder
    {
        $query = $model->select('cuti.*')->with([
            'user.formasi_tim.tim.seksi',
            'user.jabatan',
            'user.formasi_tim.pulau',
            'jenis_cuti',
        ...
        {
            $query->whereRelation('user.formasi_tim', 'pulau_id', '=', $this->pulau_id);
        }
        ...
        if($this->status_cuti_id != null)
        {
            $query->where('status_cuti_id', $this->status_cuti_id);
        }
