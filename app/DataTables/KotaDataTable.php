...
class KotaDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('aksi', function ($item) {
                $editRoute = route('kota.update', $item->uuid);
                $editButton = "
                    <button class='btn btn-outline-primary' data-toggle='modal'
                            ...
                            data-name='{$item->name}'
                            data-code='{$item->code}'
                            data-provinsi_id='{$item->provinsi_id}'
                            >
                        <i class='fa fa-edit'></i>
                    </button>";

                $deleteRoute = route('kota.destroy', $item->uuid);
                $deleteButton = "
    ...
    public function query(Kota $model): QueryBuilder
    {
        $query = $model->select('kota.*')->with([
            'provinsi',
        ])->newQuery();
